...
import { hasErrored, isLoading, useAsyncRequest } from "./useAsyncRequest.js";
...
describe("useAsyncRequest Coverage Suite", () => {
  ...
  it("useAsyncRequest: covers success path with 1 param", async () => {
    const mockReq = vi.fn().mockResolvedValue("success");
    const { result } = renderHook(() => useAsyncRequest(mockReq, "p1"));

    await waitFor(() => expect(result.current.state).toBe("succeeded"));
  });

  it("useAsyncRequest: covers success path with 2 params", async () => {
    const mockReq = vi.fn().mockResolvedValue("success");
    const { result } = renderHook(() => useAsyncRequest(mockReq, "p1", "p2"));

    await waitFor(() => expect(result.current.state).toBe("succeeded"));
  });

  it("useAsyncRequest: covers success path with 3 params", async () => {
    const mockReq = vi.fn().mockResolvedValue("success");
    const { result } = renderHook(() => useAsyncRequest(mockReq, "p1", "p2", "p3"));

    await waitFor(() => expect(result.current.state).toBe("succeeded"));
  });

  it("useAsyncRequest: covers success path with 4 params", async () => {
    const mockReq = vi.fn().mockResolvedValue("success");
    const { result } = renderHook(() => useAsyncRequest(mockReq, "p1", "p2", "p3", "ref"));
...
describe("useAsyncRequest Hooks", () => {
  it("useAsyncRequest: resolves in StrictMode once an async request completes", async () => {
    const mockRequest = vi.fn().mockImplementation(
      () =>
        new Promise<string>((resolve) => {
          setTimeout(() => resolve("strict mode success"), 0);
        }),
    );
  ...
  it("useAsyncRequest: should re-run when param changes", async () => {
    const mockRequest = vi.fn().mockImplementation((p: string) => Promise.resolve(`Result: ${p}`));
    const { result, rerender } = renderHook(({ param }) => useAsyncRequest(mockRequest, param), {
  ...
  it("useAsyncRequest: should pass both params to the request", async () => {
    const mockRequest = vi.fn().mockResolvedValue("ok");

    renderHook(() => useAsyncRequest(mockRequest, "first", 2));
    expect(mockRequest).toHaveBeenCalledWith("first", 2);
  });

  it("useAsyncRequest: should pass all params to the request", async () => {
    const mockRequest = vi.fn().mockResolvedValue("ok");

    renderHook(() => useAsyncRequest(mockRequest, "first", 2, "third"));
...
describe("useAsyncRequest Hooks - Error Handling", () => {
  ...
  it("useAsyncRequest should catch error with 1 param", async () => {
    const mockRequest = vi.fn().mockRejectedValue(new Error("Param 1 Error"));
    const { result } = renderHook(() => useAsyncRequest(mockRequest, "test"));
  ...
  it("useAsyncRequest should catch error with 2 params", async () => {
    const mockRequest = vi.fn().mockRejectedValue(new Error("Param 2 Error"));
    const { result } = renderHook(() => useAsyncRequest(mockRequest, "p1", "p2"));
  ...
  it("useAsyncRequest should catch error with 3 params", async () => {
    const mockRequest = vi.fn().mockRejectedValue(new Error("Param 3 Error"));
    const { result } = renderHook(() => useAsyncRequest(mockRequest, "a", "b", "c"));
  ...
  it("useAsyncRequest should catch error and use refreshParam", async () => {
    const mockRequest = vi.fn().mockRejectedValue(new Error("Refresh Error"));
    const { result, rerender } = renderHook(
      ({ refresh }) => useAsyncRequest(mockRequest, "a", "b", "c", refresh),
      {
