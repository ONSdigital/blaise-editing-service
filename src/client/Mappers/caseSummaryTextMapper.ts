...
export default function mapCaseSummaryText(caseSummary: CaseSummaryDetails): string {
  ...
  caseSummaryText += '\n';
  caseSummaryText += '\n';
  caseSummaryText += `Case ID:            ${caseSummary.CaseId ? caseSummary.CaseId : ''}\n`;
  caseSummaryText += `Outcome:            ${caseSummary.OutcomeCode ? caseSummary.OutcomeCode : ''}\n`;
  caseSummaryText += `Interview date:     ${caseSummary.InterviewDate ? caseSummary.InterviewDate.toDateString() : ''}\n`;
  caseSummaryText += `District:           ${caseSummary.District ? caseSummary.District : ''}\n`;
  caseSummaryText += `Interviewer:        ${caseSummary.InterviewerName ? caseSummary.InterviewerName : ''}\n`;
  caseSummaryText += '\n';
  ...
  caseSummary.Respondents.forEach((respondent) => {
    caseSummaryText += `${PadString((respondent.PersonNumber ? respondent.PersonNumber : ''), 4)}|`;
    caseSummaryText += `${PadString(respondent.RespondentName ? respondent.RespondentName : '', nameColumnWidth)}|`;
    caseSummaryText += `${PadString(respondent.BenefitUnit ? respondent.BenefitUnit : '', 4)}|`;
    caseSummaryText += `${PadString(respondent.Sex ? respondent.Sex : '', 5)}|`;
